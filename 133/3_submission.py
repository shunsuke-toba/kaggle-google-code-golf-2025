def p(g):
 ...
 B=min(C:=[f(i//w,i%w)for i in r(w*l(g))if g[i//w][i%w]],key=lambda c:(l(c)<3,l(c)));Y,X,p=min(B,key=lambda t:sum(c==t[2]for*_,c in B))
 for c in C:
  a,b=min(A:=[(y,x)for y,x,t in c if t==p or(q:=t)*0])
  for y,x,t in B:
