def p(g):
 R=range;l=len;H,W=l(g),l(g[0]);p=next(x for r in R(H)for c in R(W)for s in R(H,r,-1)for t in R(W,c,-1)if{1}<{*sum(x:=[a[c:t]for a in g[r:s]],[])}and all(map(any,x+[*zip(*x)])))
