def p(g):
 ...
 for k in range(n*n):
  ...
  if(c:=o[i][j])>0:
   ...
   y,x=next(((y,x)for(y,x),v in s if m==v*any(n>y+a>-1<x+b<n and 0<(c:=abs(o[y+a][x+b]))!=m for a,b in d if a*b==0)));P=[(r-y,c-x)for(r,c),v in s if v==m]
   for a,b in d:
    if n>(Y:=y+a)>-1<(X:=x+b)<n and 0<(c:=abs(o[Y][X]))!=m:
     for u,v in P:
      if n>(V:=X+(v,-v)[b%2])>-1<(U:=Y+(u,-u)[a%2])<n:g[U][V]=c
 return g